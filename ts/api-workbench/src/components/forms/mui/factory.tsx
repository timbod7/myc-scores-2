...
export class UiFactory implements Factory<RenderFn> {
  ...
  renderUnionEditor(props: UnionEditorProps<RenderFn>): RenderFn {
    return (rprops: RenderProps) => {
      ...
      const underlying =
        props.veditor && props.veditor.veditor.render(props.veditor.state, props.veditor.onUpdate)(rprops);
      return {
        element: () => {
          return (
            <div>
      ...
      return {
        element: () => {
          return (
            <div>
              {toggle}
              {props.isActive && underlying.element()}
            </div>
          );
        },
        gridElement: () => {
  ...
  renderUnimplementedEditor(props: UnimplementedEditorProps): RenderFn {
    return (_rprops: RenderProps) =>
      wideFieldElement(<div>unimplemented veditor for {typeExprToStringUnscoped(props.typeExpr)}</div>);
  }
